        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex gap-2">
              <Input value={newName} onChange={e => setNewName(e.target.value)} placeholder="施術名" className="flex-1" onKeyDown={e => e.key === "Enter" && (e.preventDefault(), addMenu())} />
              <Input type="number" value={newPrice} onChange={e => setNewPrice(e.target.value)} placeholder="料金" className="w-28" />
              <span className="text-sm text-muted-foreground self-center">円</span>
              <Button type="button" variant="outline" onClick={addMenu}><Plus className="w-4 h-4" /></Button>
            </div>
          </CardContent>
        </Card>
